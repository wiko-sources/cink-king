...
extern void mt_set_emi_try (int sencond);
extern void get_dram_rank_size (int dram_rank_size[]);
extern int get_dram_rank_nr (void);
extern void get_dram_id(char* id1);
int mem_second_rank_test(void)
{
   int ret=0;
   int i=0;
   int dram_rank_size[4];
   char id[16];
   const char  hynix_ID[12]={0x90,0x01,0x4A,0x20,0x58,0x49,0x4E,0x59,0x48,0x0,0x0,0x0};
   get_dram_id(id); 
   get_dram_rank_size (dram_rank_size);
   print ("[%s] mem_second_rank_test  dram_rank_size=%x\n", MOD, dram_rank_size[0]);
   if((get_dram_rank_nr()==1)&&(memcmp(id, hynix_ID, 9) == 0)){
     mt_set_emi_try(1);
   	if ((i = complex_mem_test ((0x0+dram_rank_size[0]), MEM_TEST_SIZE)) == 0)
        {
            print ("[%s] complex R/W mem1 test pass\n", MOD);
	     ret=0;
        }
        else
        {
            print ("[%s] complex R/W mem1 test fail :%x\n", MOD, i);
            ret=1;
        }
