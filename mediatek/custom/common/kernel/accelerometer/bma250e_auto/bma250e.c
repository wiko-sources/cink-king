...
{
	u8 databuf[2] = {0};    
	int res = BMA250E_SUCCESS;
	u8 addr = BMA250E_REG_POWER_CTL;
	struct bma250e_i2c_data *obj = i2c_get_clientdata(client);
	int retry = BMA250E_RETRY_MAX;
	...
	do
	{
		...
		res = BMA250E_SUCCESS;
		mdelay(20);
	} while ((retry-- > 0) && (res != BMA250E_SUCCESS));

	GSE_LOG("res = %d by retry %d times!\n", res, (BMA250E_RETRY_MAX - retry));
...
static int BMA250E_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	do
	{
		...
		{
			GSE_ERR("bma250e read Dataformat failed \n");
			res = BMA250E_ERR_I2C;
			continue;
		}
...
static int BMA250E_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	do
	{
		...
		{
			GSE_ERR("bma250e read rate failed\n");
			res = BMA250E_ERR_I2C;
			continue;
		}
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = BMA250E_SetPowerMode( priv->client, !power_mode);
				}
			}
			break;
