...
struct i2c_driver tpd_i2c_driver = {
    ...
    .id_table = ft5306_i2c_id,
//    .address_data = &addr_data,
};

static int tpd_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info) {
    TPD_DEBUG("tpd_i2c_detect\n");
    strcpy(info->type, "ft5306");
    return 0;
}
...
//Ivan
    ...
    {
//      if(phonestate)
//        printk("Ivan:tpd_downx=%d,y=%d,phonestate=%d\n",x,y,phonestate);
  
//Ivan        input_report_abs(tpd->dev, ABS_PRESSURE, 128);
	TPD_DEBUG("tpd_down!\n");
...
int tpd_up(int raw_x, int raw_y, int x, int y,int *count) {
    if(tpd_status){
        //Ivan
          if(phonestate)
...
int tinno_ts_parase_data(struct touch_info *cinfo, struct touch_info *sinfo, uint8_t buffer[])
{
    ...
    {
	if (report_data.fingers > 0 && report_data.fingers < 3 )
	    tpd_dcount = 1;
	else
	    tpd_dcount = 0;
    }
       ...
       if(tpd_dcount){		
		for ( i=0; i < TINNO_TOUCH_TRACK_IDS; i++ ){
			if (report_data.xy_data[i].event_flag != FTS_EF_RESERVED) {
				...
				cinfo->pt[iTouchID].x = report_data.xy_data[i].x_h << 8 | report_data.xy_data[i].x_l ;
				cinfo->pt[iTouchID].y = report_data.xy_data[i].y_h << 8 | report_data.xy_data[i].y_l ;
				cinfo->pt[iTouchID].z = report_data.xy_data[i].event_flag;	
				cinfo->pt[iTouchID].id = report_data.xy_data[i].touch_id;		
				TPD_DEBUG("Point ID == %d, x == %d , y == %d \n!",cinfo->pt[iTouchID].id,cinfo->pt[iTouchID].x,cinfo->pt[iTouchID].y);
...
int tpd_gettouchinfo(struct touch_info *cinfo, struct touch_info *sinfo) {
    ...
    ret = tinno_ts_parase_data(cinfo, sinfo, buffer);
	return ret;
}
...
//Ivan ++
	if (tpd_init_skip) {tpd_init_skip = 0; continue; }
       ...
       if(cinfo.count==-1) continue;
		...
		if (cinfo.count > 0 && cinfo.count < 3)
		{
		    if(tpd_downMode != 2)		//Don't accept touch point in virtual key mode
		    {

			if (cinfo.pt[0].z == FTS_EF_DOWN || cinfo.pt[0].z == FTS_EF_CONTACT)
			{
			    tpd_down(cinfo.pt[0].x , cinfo.pt[0].y, cinfo.pt[0].x , cinfo.pt[0].y, 1);
//Ivan			    input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, cinfo.pt[0].id);
			}
                    else if(cinfo.pt[0].z == FTS_EF_UP)
                       {
                               tpd_up(cinfo.pt[0].x , cinfo.pt[0].y, cinfo.pt[0].x , cinfo.pt[0].y, NULL);
//Ivan                               input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, cinfo.pt[0].id);
                       }
			if (cinfo.pt[1].z == FTS_EF_DOWN || cinfo.pt[1].z == FTS_EF_CONTACT)
			{
			    tpd_down(cinfo.pt[1].x , cinfo.pt[1].y, cinfo.pt[1].x , cinfo.pt[1].y, 1);
//Ivan			    input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, cinfo.pt[1].id);
			}
                    else if(cinfo.pt[1].z == FTS_EF_UP)
                       {
                               tpd_up(cinfo.pt[1].x , cinfo.pt[1].y, cinfo.pt[1].x , cinfo.pt[1].y, NULL);
//Ivan                               input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, cinfo.pt[1].id);
                       }
			tpd_downMode = 1;					//Change to touch mode
		//	input_sync(tpd->dev);
		    }
		}
		else if (cinfo.count == 3)					//Virtual key Down 1
		{	    
		    if (tpd_downMode != 1)					//Don't accept virtual key in touch mode
		    {
				
			    TPD_DEBUG("TP virtual key pressed = %d \r\n",cinfo.pt[0].x);
			    tpd_down(cinfo.pt[0].x , cinfo.pt[0].y, cinfo.pt[0].x , cinfo.pt[0].y, 1);
//Ivan			    input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, cinfo.pt[0].id);
				if(tp_boot_mode == FACTORY_BOOT || tp_boot_mode == RECOVERY_BOOT){
				    TPD_DEBUG("TP HW key pressed = %d \r\n",cinfo.pt[0].x);
				    tpd_button(cinfo.pt[0].x, cinfo.pt[0].y, 1);
				}
				tpd_downMode = 2;					//Change to virtual key mode
			//	input_sync(tpd->dev);
		    }
		}
		else if (cinfo.count == 4)					//Virtual key Down 2
		{
		    if (tpd_downMode != 1)					//Don't accept virtual key in touch mode
		    {
			TPD_DEBUG("TP virtual key pressed = %d \r\n",cinfo.pt[1].x);
			tpd_button(cinfo.pt[1].x, cinfo.pt[1].y, 1);
			tpd_downMode = 2;					//Change to virtual key mode
		//	input_sync(tpd->dev);
		    }
		}
		else if (cinfo.count == 5)					//Virtual key Up 1
		{
		    if (tpd_downMode != 1)					//Don't accept virtual key in touch mode
		    {		    
			TPD_DEBUG("TP virtual key release = %d \r\n",cinfo.pt[0].x);
			tpd_button(cinfo.pt[0].x, cinfo.pt[0].y, 0);
			tpd_downMode = 2;					//Change to virtual key mode			
			//input_sync(tpd->dev);
		    }
		}
		else if (cinfo.count == 6)					//Virtual key Up 2
		{
		    if (tpd_downMode != 1)					//Don't accept virtual key in touch mode
		    {
			TPD_DEBUG("TP virtual key release = %d \r\n",cinfo.pt[1].x);
			tpd_button(cinfo.pt[1].x, cinfo.pt[1].y, 0);
			tpd_downMode = 2;					//Change to virtual key mode		
		//	input_sync(tpd->dev);
		    }
		}
...
//			if (down > 0)
//			{
				TPD_DEBUG("TP Release\n");
				
				if(tp_boot_mode == FACTORY_BOOT || tp_boot_mode == RECOVERY_BOOT && tpd_downMode == 2)
				{
				    tpd_button(cinfo.pt[0].x, cinfo.pt[0].y, 0);
				    input_mt_sync(tpd->dev);
				}
...
//edit by Magnum 2012-3-20
    ...
    if(tpd_down_state){
        tpd_up(down_x,down_y,down_x,down_y,NULL);
...
//	return;
    ...
    tpd_downMode = 0;
/*	
    mt_set_gpio_out(TPIO_WAKEUP,0);				//Power down pin output low
    msleep(10);
    mt_set_gpio_out(TPIO_WAKEUP,1);				//Power down pin output High
    msleep(10);

//    i2c_master_send(i2c_client,wakeup,2);
	i2c_smbus_write_byte_data(i2c_client, 0xA5, 0x00);
*/
    tinno_tp_power_on();
...
/*	    
	    ...
	    i2c_smbus_write_byte_data(ts->client, 0xA5, 0x00);
*/	    
	    tinno_tp_power_on();
...
//	mutex_lock(&tp_mutex);
	...
	msleep(50);
#if 0
	/*check the register 0xfc */
	ret = i2c_smbus_read_byte_data(ts->client, command);
	if (ret < 0) {
		TPD_DEBUG("I2C transfer failed 1!(%d)\n", ret);
		goto err;
	}
	if ( 0xAA != ret ){
		TPD_DEBUG("FTS mode switch error 2!(%d)\n", ret);
		msleep(100);
	}
#endif	
	/*write 0x55 to register 0xfc*/
	...
	ret = i2c_smbus_read_byte_data(ts->client, command);
	if (ret < 0) {
		TPD_DEBUG("I2C transfer failed 3!(%d)\n", ret);
		goto err;
	}
	if ( 0x01 != ret ){
		TPD_DEBUG("FTS mode switch error 4!(%d)\n", ret);
		ret = -EAGAIN;
		goto err;
	}
	TPD_DEBUG("FTS reset OK!(%d)\n", ret);
#endif	
	msleep(40);
