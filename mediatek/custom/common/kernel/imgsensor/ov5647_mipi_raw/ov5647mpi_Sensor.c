...
MSDK_SCENARIO_ID_ENUM OV5647CurrentScenarioId = MSDK_SCENARIO_ID_CAMERA_PREVIEW;
...
extern bool OV5647MIPI_otp_update_wb(unsigned char golden_rg, unsigned char golden_bg);
extern signed char OV5647MIPI_otp_read_wb_group(signed char index);
...
static void OV5647MIPI_Write_Shutter(kal_uint16 iShutter)
{
	...
	if(OV5647MIPIAutoFlicKerMode){
		if(OV5647MIPI_sensor.video_mode == KAL_FALSE){
		   if(OV5647CurrentScenarioId== MSDK_SCENARIO_ID_CAMERA_ZSD)
		   {
...
//	kal_sleep_task(2);
	...
	mdelay(50);
	if(OV5647MIPI_otp_update_wb(0x45, 0x67))
	{
		SENSORDB("mingji test truly module wb update success ***************************************\n");
	}
...
UINT32 OV5647MIPIOpen(void)
{
	...
	SENSORDB("mingji test OV5647MIPIOpen, this sensor vendor is truly, sensor_id:%x \n",sensor_id);
...
UINT32 OV5642GetSensorID(UINT32 *sensorID) 
{
	...
	*sensorID=((OV5647MIPI_read_cmos_sensor(0x300A) << 8) | OV5647MIPI_read_cmos_sensor(0x300B));	
	...
	{
	    SENSORDB("mingji test OV5647GetSensorID,this is sunny or darling module.\n");
            *sensorID = 0xFFFFFFFF;
            return ERROR_SENSOR_CONNECT_FAIL;
	}
	else
	{
	    SENSORDB("mingji test OV5647GetSensorID,this is truly module.\n");
	}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    OV5647CurrentScenarioId =ScenarioId;
	switch (ScenarioId)
	{
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_GET_PERIOD:	/* 3 */
			switch(OV5647CurrentScenarioId)
			{
				case MSDK_SCENARIO_ID_CAMERA_ZSD:
