...
#define _DELAY_HDST_ 	4	//20us              //START HOLD,STOP SETUP,REPEAT SETUP BUS FREE Time Loop times to delay 20uS (Need to change according to system clock)
//#define _DELAY_FULL_ 	20	//10us//95          //FULL CLOCK TIME
#define _DELAY_HALF_ 	2	//5us//45           //HALF CLOCK TIME
...
//   	udelay(_DELAY_FULL_);
   ...
   udelay(_DELAY_HDST_);
   mt_set_gpio_out(SW_I2C_SCL, 1);	//release SDA to receive ack
//   	udelay(_DELAY_FULL_);
   udelay(_DELAY_HALF_);
   ack = mt_get_gpio_in(SW_I2C_SDA); /// ack
   mt_set_gpio_out(SW_I2C_SCL, 0);
...
//    sw_iic_tx_byte(tn_camera_i2c_addr + 1);    /// device ID and R/W bit
    ack = sw_iic_tx_byte(a_u2Addr + 1);    /// device ID and R/W bit
    if (ack)
	goto Err_exitRX;
    for (i = 0; i < size; i++)
    {
//Ivan
	if (i == (size - 1))
	    sw_iic_rx_byte(&a_puBuff[i], 1);        /// read data and send ACK 1 to indicate end of data read
	else
	    sw_iic_rx_byte(&a_puBuff[i], 0);        /// read data and send ACK 0 to indicate more data to read
...
/* SW_I2C_ReadReg */
/*=============================================================================*/
int SW_I2C_WriteReg(u16 a_u2Addr , u8* a_u4Data , u32 a_u4Bytes, int send_stop)
{
    int i;
    int ack, sum_ack;
    sum_ack = 0;
...
//    sw_iic_tx_byte(tn_camera_i2c_addr);   /// device ID and R/W bit
    sum_ack = sw_iic_tx_byte(a_u2Addr);   /// device ID and R/W bit
    if (sum_ack)
	goto Err_exitTX;
    for (i = 0; i < a_u4Bytes; i++)
    {
//	printk("SW_I2C_WriteReg data %d = %x \n", i, a_u4Data[i]);
	ack = sw_iic_tx_byte(a_u4Data[i]);             /// control word
	if (i < (a_u4Bytes - 1))			//FIXME Ivan ignore ack from last byte (OV sensor special request??)???
	    sum_ack += ack;
	if (sum_ack)
	    goto Err_exitTX;
    }
...
#define TP_I2C_SCL GPIO122
#define TP_I2C_SDA GPIO112
#define CAM_I2C_SCL GPIO222
#define CAM_I2C_SDA GPIO224
extern s32 mt_set_gpio_mode(u32 u4Pin, u32 u4Mode);
extern s32 mt_set_gpio_out(u32 u4Pin, u32 u4PinOut);
extern s32 mt_set_gpio_dir(u32 u4Pin, u32 u4Dir);

void cam_i2c1_mode_set(int enable)
{
    if(enable){
        mt_set_gpio_out(TP_I2C_SCL,GPIO_OUT_ONE);
        mt_set_gpio_out(TP_I2C_SDA,GPIO_OUT_ONE);
        mt_set_gpio_dir(TP_I2C_SCL,GPIO_DIR_IN);
        mt_set_gpio_dir(TP_I2C_SDA,GPIO_DIR_IN);
	    mt_set_gpio_mode(TP_I2C_SCL, GPIO_MODE_00);		//GPIO mode
    	mt_set_gpio_mode(TP_I2C_SDA, GPIO_MODE_00);		//GPIO mode
    	
        mt_set_gpio_out(CAM_I2C_SCL,GPIO_OUT_ONE);
        mt_set_gpio_out(CAM_I2C_SDA,GPIO_OUT_ONE);
        mt_set_gpio_dir(CAM_I2C_SCL,GPIO_DIR_OUT);
        mt_set_gpio_dir(CAM_I2C_SDA,GPIO_DIR_OUT);
	    mt_set_gpio_mode(CAM_I2C_SCL, GPIO_MODE_01);		//I2C mode
        mt_set_gpio_mode(CAM_I2C_SDA, GPIO_MODE_01);		//I2C mode
        udelay(10);
   
    }
    else{
        mt_set_gpio_dir(CAM_I2C_SCL,GPIO_DIR_IN);
        mt_set_gpio_dir(CAM_I2C_SDA,GPIO_DIR_IN); 
        mt_set_gpio_out(CAM_I2C_SCL,GPIO_OUT_ONE);
        mt_set_gpio_out(CAM_I2C_SDA,GPIO_OUT_ONE);
	    mt_set_gpio_mode(CAM_I2C_SCL, GPIO_MODE_00);		//GPIO mode
        mt_set_gpio_mode(CAM_I2C_SDA, GPIO_MODE_00);		//GPIO mode
    	
        mt_set_gpio_out(TP_I2C_SCL,GPIO_OUT_ONE);
        mt_set_gpio_out(TP_I2C_SDA,GPIO_OUT_ONE);
        mt_set_gpio_dir(TP_I2C_SCL,GPIO_DIR_OUT);
        mt_set_gpio_dir(TP_I2C_SDA,GPIO_DIR_OUT);
		mt_set_gpio_mode(TP_I2C_SCL, GPIO_MODE_04);		//I2C mode
        mt_set_gpio_mode(TP_I2C_SDA, GPIO_MODE_04);		//I2C mode
...
void I2C1_Path_select(u32 slave_address)
{
#ifndef TINNO_SW_I2C_IMPL  //elan tpd 0x2a,ov2659 0x60,imx105 0x34,hi253 0x40,hi704 0x60,fm50af 0x18,ov8825 0x6c,sp0a19 0x42 gc0329 0x62
    if((slave_address==(0x34>>1))||(slave_address==(0x60>>1))
	||(slave_address==(0x40>>1))||(slave_address==(0x6c>>1))
	||(slave_address==(0x18>>1))||(slave_address==(0x42>>1))
	||(slave_address==(0x62>>1))){
        cam_i2c1_mode_set(1);
    }else{
        cam_i2c1_mode_set(0);
    }
...
EXPORT_SYMBOL(I2C1_Path_select);
...
int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId)
{
    ...
    //
#ifdef TINNO_SW_I2C_IMPL
    i4RetValue = SW_I2C_WriteReg(i2cId, puReadCmd, 2, NO_STOP);
#else
...
int iReadRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u8 * a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
    ...
    //
#ifdef TINNO_SW_I2C_IMPL
    i4RetValue = SW_I2C_WriteReg(i2cId, a_pSendData, a_sizeSendData, NO_STOP);
#else
...
//    PK_DBG("Addr : 0x%x,Val : 0x%x \n",a_u2Addr,a_u4Data);
    ...
    do {
#ifdef TINNO_SW_I2C_IMPL
        i4RetValue = SW_I2C_WriteReg(i2cId, puSendCmd, (a_u4Bytes + 2),SEND_STOP);
#else
...
int iBurstWriteReg(u8 *pData, u32 bytes, u16 i2cId) 
{
    ...
    do {
#ifdef TINNO_SW_I2C_IMPL
        ret = SW_I2C_WriteReg(i2cId, pData, bytes,SEND_STOP);
#else
...
{	
	int ret = 0;		
#ifdef TINNO_SW_I2C_IMPL
    ret = SW_I2C_WriteReg(g_pstI2Cclient->addr, pData, lens,SEND_STOP);
#else
...
int iMultiWriteReg(u8 *pData, u16 lens)
{
  ...
  if (transac_len > MAX_CMD_LEN) {
	...
	g_pstI2Cclient->addr = (((g_pstI2Cclient->addr >> 1) & I2C_MASK_FLAG) | I2C_DMA_FLAG);
#ifdef TINNO_SW_I2C_IMPL
    ret = SW_I2C_WriteReg(g_pstI2Cclient->addr, dma_addr, lens,SEND_STOP);
#else		
...
//    PK_DBG("Addr : 0x%x,Val : 0x%x \n",a_u2Addr,a_u4Data);
    ...
    do {
#ifdef TINNO_SW_I2C_IMPL
        i4RetValue = SW_I2C_WriteReg(i2cId, a_pSendData, a_sizeSendData,SEND_STOP);
#else
        i4RetValue = i2c_master_send(g_pstI2Cclient, a_pSendData, a_sizeSendData);
#endif
        if (i4RetValue != a_sizeSendData) {
...
static ssize_t tinno_camera_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%s", g_currSensorName);
}
