...
kal_uint32 HI253_pv_HI253_exposure_lines = 0x04ed68,HI253_cp_HI253_exposure_lines=0;
...
{ 
	...
	//HI253WriteCmosSensor(0x62, 0x84);// 76//80 //2012-11-08 mingji modify
       HI253WriteCmosSensor(0x61, 0x76);// 74
	HI253WriteCmosSensor(0x62, 0x7a);// 76//80
	...
	HI253WriteCmosSensor(0x50, 0xe6);//e6
...
HI253WriteCmosSensor(0x52, 0xb0); //b0
	...
	HI253WriteCmosSensor(0xb2, 0xc5); //d0
...
HI253_Init_Cmds();
SENSORDB("[MINGJI]HI253InitSetting() end;\n");
...
void HI253NightMode(kal_bool Enable)
{
  ...
  if(Enable)
  {
    if (HI253Status.Banding == AE_FLICKER_MODE_50HZ) 
    {
	  ...
	  HI253WriteCmosSensor(0x40, 0x00); //Hblank 232
	  HI253WriteCmosSensor(0x41, 0xe8); 
	  ...
	  HI253WriteCmosSensor(0x83, 0x09);//0x015f00
	  HI253WriteCmosSensor(0x84, 0xda);
	  HI253WriteCmosSensor(0x85, 0xd0);
	...
	else
	{
	  ...
	  HI253WriteCmosSensor(0x83, 0x09);//5fps
	  HI253WriteCmosSensor(0x84, 0xda);
	  HI253WriteCmosSensor(0x85, 0xd0);
	  ...
	  HI253WriteCmosSensor(0x88, 0x09);//5fps
	  HI253WriteCmosSensor(0x89, 0xdA);
	  HI253WriteCmosSensor(0x8a, 0xd0);
  ...
  else
  {
    if (HI253Status.Banding == AE_FLICKER_MODE_50HZ) 
	{
	  ...
	  HI253WriteCmosSensor(0x88, 0x05); //0x05e800//06
	  HI253WriteCmosSensor(0x89, 0xe9); //cc mj
	  HI253WriteCmosSensor(0x8a, 0xb0); //3c mj
	...
	else
	{
	  ...
	  HI253WriteCmosSensor(0x40, 0x00); //Hblank 232
	  HI253WriteCmosSensor(0x41, 0xe8); 
	  HI253WriteCmosSensor(0x42, 0x00); //Vblank 141
	  HI253WriteCmosSensor(0x43, 0x8d);
	  HI253_Sleep_Mode = (HI253ReadCmosSensor(0x01) & 0xfe);
	  HI253_Sleep_Mode |= 0x01;
	  HI253WriteCmosSensor(0x01, HI253_Sleep_Mode);
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
  ...
  HI253Status.MaxFrameRate = HI253_MAX_FPS;
  spin_unlock(&hi253_drv_lock);
  HI253SetMirror(IMAGE_NORMAL);//IMAGE_HV_MIRROR
  ...
  HI253WriteCmosSensor(0x83, HI253_pv_HI253_exposure_lines >> 16);
  HI253WriteCmosSensor(0x84, (HI253_pv_HI253_exposure_lines >> 8) & 0x000000FF);
  HI253WriteCmosSensor(0x85, HI253_pv_HI253_exposure_lines & 0x000000FF);
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
  ...
  {
      HI253SetPage(0x20);
      HI253_cp_HI253_exposure_lines=(HI253ReadCmosSensor(0x80) << 16)|(HI253ReadCmosSensor(0x81) << 8)|HI253ReadCmosSensor(0x82);
      spin_lock(&hi253_drv_lock);
	  HI253_pv_HI253_exposure_lines=HI253_cp_HI253_exposure_lines;
      HI253Status.PvShutter = HI253_cp_HI253_exposure_lines;  
      spin_unlock(&hi253_drv_lock);
  }
...
BOOL HI253SetWb(UINT16 Para)
{
  ...
  switch (Para)
  {
    ...
    case AWB_MODE_AUTO:
		...
		HI253WriteCmosSensor(0x11, 0x2e);		
		HI253WriteCmosSensor(0x83, 0x53);
		HI253WriteCmosSensor(0x84, 0x16);
...
/*  HI253SetPage(0x10);  
  ...
  switch (Para)
  {
    ...
    case AE_EV_COMP_00:                /* EV 0 */
      HI253WriteCmosSensor(0x40,0x88);//84
      break;
